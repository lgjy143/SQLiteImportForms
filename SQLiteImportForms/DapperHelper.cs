...
using System.Data;
using static SQLiteImportForms.ConnectionFactory;
...
namespace SQLiteImportForms
{
    ...
    public class DapperHelper : IDapperHelper, IDisposable
    {
        ...
        //public DapperHelper(string conn = "", string jsonConfigFileName = "App.config", DatabaseType databaseType = DatabaseType.Oracle)
        //{
        ...
        //    conn = System.Configuration.ConfigurationManager.AppSettings["DefaultConnect"].ToString();
        ...
        /// <summary>
        /// 初始化 若不传则默认从App.config读取DefaultConnect节点
        /// 直接传入连接串则
        /// </summary>
        /// <param name="conn"></param>
        public DapperHelper(string conn = "", DatabaseType databaseType = DatabaseType.Oracle)
        {
            if (string.IsNullOrEmpty(conn))
            {
                conn = System.Configuration.ConfigurationManager.AppSettings["DefaultConnection"].ToString();
            }
            ConnectionString = conn;
            Connection = ConnectionFactory.CreateConnection(ConnectionString, databaseType);
        }
        ...
        public DataTable QueryDataTable(string sql, object param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null)
        {
            var dt = new DataTable();

            var listDataReader = Connection.Connection.ExecuteReader(sql, param, transaction, commandTimeout, commandType);

            if (listDataReader != null) dt.Load(listDataReader);
